...
While the conditioning signal is not always strong enough to enforce the correct subtraction result, the model consistently produces structured, plausible digits and occasionally solves the subtraction task correctly. In the examples folder, you can find image grids where the first two columns are the conditioning digits and the third column is the DDPM generated digit. This behavior highlights known limitations of early conditioning strategies in diffusion models and motivates architectural extensions such as multi-scale conditioning or classifier-free guidance.
